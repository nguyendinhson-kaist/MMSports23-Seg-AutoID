...
class _SwinTransformer(SwinTransformer):
    def __init__(self, depths=(2, 2, 6, 2), **kwargs):
        super().__init__(depths=depths, **kwargs)
    ...
    def forward(self, x, cb_feats=None, pre_tmps=None):
        ...
        if hasattr(self, 'patch_embed'):
            ...
            tmps.append((x, hw_shape))
        else:
            x, hw_shape = pre_tmps[0]

        for i in range(self.num_layers):
            stage = self.stages[i]
            if stage is None:
                x, hw_shape, out, out_hw_shape = pre_tmps[i+1]
            else:
                if cb_feats is not None:
                    x = x + cb_feats[i]
                x, hw_shape, out, out_hw_shape = stage(x, hw_shape)
            tmps.append((x, hw_shape, out, out_hw_shape))
