...
class MSHTCRoIHead(HybridTaskCascadeRoIHead):
    ...
    def __init__(self,
                 mask_iou_head: OptMultiConfig,
                 mask_head: OptMultiConfig = None, 
                 **kwargs) -> None:
        super().__init__(mask_head=mask_head, **kwargs)
        assert isinstance(mask_iou_head, list)
        assert len(mask_iou_head) == len(mask_head)

        self.mask_iou_head = ModuleList()
        for head in mask_iou_head:
            self.mask_iou_head.append(MODELS.build(head))
    ...
    def _mask_forward(self,
        ...
        if training:
            ...
            mask_results = dict(mask_preds=aug_masks, mask_feats=mask_feats)
    ...
    def mask_loss(self,
        ...
        mask_iou_pred = mask_iou_head(mask_results['mask_feats'],
                                           pos_mask_pred)
        pos_mask_iou_pred = mask_iou_pred[range(mask_iou_pred.size(0)),
                                          pos_labels]
        
        loss_mask_iou = mask_iou_head.loss_and_target(
            pos_mask_iou_pred, pos_mask_pred, mask_targets, sampling_results,
            batch_gt_instances, self.train_cfg[stage])
    ...
    def predict_mask(self,
        ...
        mask_iou_preds = self.mask_iou_head[-1](
        ...
        results_list = self.mask_head[-1].predict_by_feat(
            ...
            rescale=rescale,
            activate_map=True)
        results_list = self.mask_iou_head[-1].predict_by_feat(
    ...
    def forward(self, x: Tuple[Tensor], rpn_results_list: InstanceList,
        ...
        if self.with_mask:
            ...
            # mask iou head
            mask_preds = mask_results['mask_preds'][-1]
            mask_feats = mask_results['mask_feats']
            _, labels = cls_scores[:, :self.bbox_head.num_classes].max(dim=1)
            mask_iou_preds = self.mask_iou_head[-1](
                mask_feats, mask_preds[range(labels.size(0)), labels])
